...
class Attention(nn.Module):
    def __init__(self, hidden_size):
        super().__init__()
        self.Wh = nn.Linear(hidden_size, hidden_size, bias=False)  # 编码器隐状态变换
        self.Ws = nn.Linear(hidden_size, hidden_size, bias=False)  # 解码器隐状态变换
        self.v = nn.Linear(hidden_size, 1, bias=False)             # 输出注意力分数
    
    def forward(self, dec_h, enc_hs):
        # dec_h: (batch, hidden) 解码器当前隐状态
        # enc_hs: (batch, src_len, hidden) 编码器所有隐状态
        src_len = enc_hs.size(1)
        dec_h = dec_h.unsqueeze(1).expand(-1, src_len, -1)  # 扩展以便与enc_hs对齐
        # 计算注意力能量: e = v^T * tanh(Wh*h + Ws*s)
        energy = self.v(torch.tanh(self.Wh(enc_hs) + self.Ws(dec_h))).squeeze(-1)
        attn = F.softmax(energy, dim=-1)  # 归一化得到注意力权重
        # 加权求和得到上下文向量
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        bs = q.size(0)
        # 线性变换后分头: (batch, seq_len, d_model) -> (batch, num_heads, seq_len, d_k)
...
本次作业由本人独立完成（当然肯定少不了Claude的鼎力支持\^o^/，不过主要模型代码还是手动实现的），包括：
- LSTM、GRU单元的手动实现
